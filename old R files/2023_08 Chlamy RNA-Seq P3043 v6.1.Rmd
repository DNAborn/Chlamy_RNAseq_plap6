...
pdf(file="graphs2/Mapping_Rates.pdf", width=10, height=20)
...
# head(metadata(se)[["quantInfo"]])
summary(metadata(se)[["quantInfo"]]$percent_mapped)
summary(metadata(se)[["quantInfo"]]$num_processed)
summary(metadata(se)[["quantInfo"]]$percent_mapped)
summary(metadata(se)[["quantInfo"]]$num_processed)
...
getwd()
save(gse,file="tximeta.v6.1.txm")
gse <- 1
load(file="tximeta.v6.1.txm")
...
```{r}
head(rownames(dds))
ids <- str_remove(rownames(dds),pattern = "_4532")
head(ids)
rownames(dds) <- ids
...
g <- ggplot(pcaData, aes(PC1, PC2, color=condition, shape = trial)) +
  geom_point(size=3) +
  geom_text_repel(aes(label = sampleid), size = 2, max.overlaps = 20) +
...
pdf(file="graphs2/PCA.pdf", width=12, height=12)
par(mar=c(4,4,4,4)+.1)
plot(g)
dev.off()
...
pdf(file="graphs2/MA-fib.vs.WT(TAP).pdf", width=8, height=5)
plotMA(resLFC_TAP, main = "fib vs. WT in TAP", ylim=c(-4,4))
dev.off()
