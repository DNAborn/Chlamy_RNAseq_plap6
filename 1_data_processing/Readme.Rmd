...
```{r pre_trans_fig2, figures-side, fig.show="hold", out.width="33%"}
load(file=paste(datadir,"rlog.rld", sep="/"))
meanSdPlot(assay(ntd))
meanSdPlot(assay(vsd))
...
### - Check sample distance
```{r pre_sample_dist2, fig.height=12, out.width="100%"}
sampleDists <- dist(t(assay(vsd)))
...
### - Advanced PCA
```{r pca_advanced2, warning = FALSE, fig.show="hold", out.width="80%", fig.height=6}
vst_dat <- assay(vst(dds))
